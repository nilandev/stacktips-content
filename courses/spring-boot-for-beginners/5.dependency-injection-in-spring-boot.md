...
In the [previous chapter](/articles/understanding-spring-boot-project-structure), we saw that `@SpringBootApplication` bundles `@ComponentScan` to automatically detect and register your Spring beans. In this chapter, we'll unpack what that actually means — how Spring registers beans, and the different ways it injects them into the classes that need them.
...
![Spring Dependency Injection overview](/media/summernote/dependency-injection.jpg)

Any object in the Spring framework that is initialized through the Spring container is called Spring Bean.

Dependency Injection works in a two-step approach:

-   **Bean identification/registration** — telling Spring which classes instances should be instantiated and managed by spring
-   **Bean injection** — telling Spring where those beans should be plugged in

The two steps use different annotations and are easy to conflate as a beginner: registration annotations like `@Service` or `@Component` tell Spring *"this class should become a bean"*, while `@Autowired` tells Spring *"inject an already-registered bean here."* You'll always need both of these steps.
...
In Spring Boot, bean registration is typically done automatically using the `@ComponentScan` annotation rather than an explicit XML or Java configuration. As we saw in the [previous chapter](/articles/understanding-spring-boot-project-structure), `@ComponentScan` is already wrapped inside `@SpringBootApplication`.

Since every Spring Boot application's main class is already annotated with `@SpringBootApplication`, we don't have to add `@ComponentScan` explicitly. It performs component scanning starting from the package of your main class.
...
Note that the field can no longer be declared `final` here — unlike constructor injection, the field is set after the object is constructed, so Java won't allow `final` on it. This is also exactly why setter injection carries a risk that field injection and constructor injection don't: the object can be used before all of its dependencies are set, which can lead to `NullPointerExceptions`.

### Which Injection Type Should You Use?

Of the three, **constructor injection is the recommended** for required dependencies.

-   It lets you declare dependencies as `final`, so they can't accidentally be reassigned or left unset after construction.
-   A bean is either fully constructed with everything it needs, or it fails to start — you find out immediately at startup instead of getting the `NullPointerException` at runtime.
-   It's the easiest to unit test without a Spring container at all, since you can just call `new MyService(mockRepository)`

Field injection seems convenient, but it hides a class's dependencies from anyone reading its public API and makes it harder to test without using Java reflection. Use setter injection only when a dependency is optional.

> If you end up with two beans that implement the same interface, Spring won't know which one to inject and will throw an error. In such cases, you will require to use `@Qualifier` and `@Primary` annotations to resolve that. 

## Summary

In this chapter, we saw why manually wiring up dependencies gets error-prone as an application grows, and how Spring's Dependency Injection solves it through its IoC container. We covered how beans get registered and different ways to inject beans.

In the next chapter, we'll look at [Most Commonly Used Annotations in Spring Boot](/articles/spring-boot-annotations), including a closer look at the `@Component`, `@Service`, and `@Repository` stereotypes we used throughout this chapter.
